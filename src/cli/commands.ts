...
export function createProgram(): Command {
  ...
  program
    .option(
      "-f, --file <path>",
      "Name of the calendar JSON file within ./calendars/",
    )
...
async function syncCalendar(calendarFileName: string): Promise<void> {
  printInfo(`Reading waste calendar file ${calendarFileName} ...`);
  const absolutePath = path.resolve("./calendars/" + calendarFileName);
